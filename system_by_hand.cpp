...
void setup() {
  Serial.begin(9600);
  Serial.println("HOMING  | F:shelf:tray (FETCH)| S:shelf:tray (STORE) ");
    ...
    if (command.equalsIgnoreCase("HOMING")) {
      ...
      if (second_colon > first_colon) {
        ...
        if (command.startsWith("F:")) {
          ...
          moving(target_shelf, DROPOFF_Y, DROPOFF_Z,                       1, 1900);
        }
          ...
          gantryServo.write(90);
          moving(target_shelf, PICKUP_Y, PICKUP_Z[target_tray] + Z_OOFSET, 1, 2000);
          moving(target_shelf, PICKUP_Y, PICKUP_Z[target_tray] - Z_OOFSET, 0, 2000);
        }
        else {
